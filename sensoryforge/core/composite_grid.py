...
class CompositeReceptorGrid(BaseGrid):
    ...
    **Important:** This class manages receptor spatial positions ONLY. It does NOT
    ...
    def __init__(
        ...
        if xlim[0] >= xlim[1]:
            raise ValueError(f"Invalid xlim: min ({xlim[0]}) must be < max ({xlim[1]})")
        if ylim[0] >= ylim[1]:
            raise ValueError(f"Invalid ylim: min ({ylim[0]}) must be < max ({ylim[1]})")
    ...
    def add_population(
        ...
        **Deprecated:** Use `add_layer()` instead. The `filter` parameter
        ...
        self.add_layer(
            name, density, arrangement, offset=offset, color=color, **metadata
        )
