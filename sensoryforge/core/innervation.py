...
def _seeded_generator(seed: Optional[int]) -> Optional[torch.Generator]:
    """Build a fresh, per-call ``torch.Generator`` from ``seed``.

    Mirrors the pattern in :mod:`sensoryforge.filters.noise` (a per-instance
    ``torch.Generator`` instead of a global ``torch.manual_seed`` call), so
    building innervation never mutates -- or depends on -- the global RNG
    state (F-006). Returns ``None`` when ``seed`` is ``None``, in which case
    callers fall back to the global generator implicitly (unseeded, as
    before).
    """
    if seed is None:
        return None
...
class GaussianInnervation(BaseInnervation):
    ...
    def __init__(
        ...
        self.distance_weight_randomness_pct = max(
            0.0, min(100.0, distance_weight_randomness_pct)
        )
        self.seed = seed
    ...
    def compute_weights(self, **kwargs) -> torch.Tensor:
        ...
        """
        generator = _seeded_generator(self.seed)
        ...
        if self.far_connection_fraction > 0 and is_far.any():
            ...
            prob_weights = (
                1.0 - self.far_connection_fraction
            ) * local_prob + self.far_connection_fraction * far_prob
        else:
        ...
        K_per_neuron = (
            torch.poisson(poisson_tensor, generator=generator).long().to(self.device)
        )
            ...
            if is_far.any():
                ...
                n_far_per_neuron = (
                    torch.poisson(
                        torch.full(
                            (self.num_neurons,),
                            self.far_connection_fraction
                            * self.num_receptors
                            / max(1, self.num_neurons),
                            device="cpu",
                        ),
                        generator=generator,
                    )
    ...
    def compute_weights(self, **kwargs) -> torch.Tensor:
        ...
        for i in range(self.num_neurons):
            ...
            if len(all_idx) < K and len(local_idx) > 0:
                ...
                if extra:
                    all_idx = torch.cat(
                        [all_idx, torch.tensor(extra, device=self.device)]
                    )
    ...
    def compute_weights(self, **kwargs) -> torch.Tensor:
        ...
        K_per_neuron = (
            torch.poisson(
                torch.full(
                    (self.num_neurons,),
                    float(self.connections_per_neuron),
                    device="cpu",
                ),
                generator=generator,
            )
...
def create_innervation_map_tensor(
    ...
    if far_connection_fraction > 0 and is_far.any():
        ...
        far_prob = far_weights / far_sum
        flat_weights = (
            1.0 - far_connection_fraction
        ) * local_prob + far_connection_fraction * far_prob
    else:
    ...
    K_per_neuron = torch.poisson(poisson_tensor, generator=generator).long().to(device)
