...
class BatchExecutor:
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        self._is_canonical = (
            isinstance(self.base_config.get("grids"), list)
            and isinstance(self.base_config.get("populations"), list)
            and "pipeline" not in self.base_config
        )
        ...
        self.pipeline = GeneralizedTactileEncodingPipeline.from_config(self.base_config)
    ...
    def _execute_single_stimulus(
        self, stim_config: Dict[str, Any], save_intermediates: bool
    ) -> Dict[str, Any]:
    ...
    def _save_checkpoint(self, completed: set, failed: list, current_idx: int) -> None:
    ...
    def _save_results_pytorch(
        ...
        pipeline_info = (
            {
                "populations": [
                    p["name"] for p in self.base_config.get("populations", [])
                ]
            }
            if self._is_canonical
    ...
    def generate_slurm_script(
        ...
        *,
        job_name: str = "sensoryforge_batch",
        partition: str = "gpu",
        time: str = "04:00:00",
        mem_gb: int = 32,
        cpus_per_task: int = 4,
        gpus: int = 1,
        conda_env: str = "sensoryforge",
        output_dir: Optional[str] = None,
        extra_sbatch: Optional[Dict[str, str]] = None,
    ) -> str:
        """Generate a SLURM job script for this batch configuration.

        The script activates the conda environment, calls
        ``sensoryforge run`` for each stimulus index in the sweep, and
        saves results to ``output_dir``.

        Args:
            config_yaml_path: Absolute path to the batch YAML config file
                that will be passed to the CLI.
            job_name: SLURM ``--job-name``.
            partition: SLURM ``--partition``.
            time: Wall-clock limit in ``HH:MM:SS`` format.
            mem_gb: Memory in GiB (``--mem``).
            cpus_per_task: CPU cores per task (``--cpus-per-task``).
            gpus: Number of GPUs (``--gres=gpu:N``).  Set to 0 for CPU-only.
            conda_env: Name of the conda environment to activate.
            output_dir: Override the output directory.  Defaults to
                ``self.output_dir``.
            extra_sbatch: Extra ``#SBATCH`` directives as a ``{key: value}``
                dict (e.g. ``{"account": "mylab"}``).

        Returns:
            SLURM batch script as a string.  Write it to a ``.sh`` file and
            submit with ``sbatch <script>.sh``.
        """
        n_stimuli = len(self.stimulus_configs)
        ...
        lines += [
            ...
            f"#SBATCH --output={out_dir}/{job_name}_%j.out",
            f"#SBATCH --error={out_dir}/{job_name}_%j.err",
            f"#SBATCH --array=0-{n_stimuli - 1}",
        ]
        if gpus > 0:
        ...
        lines += [
            "",
            "# Auto-generated by SensoryForge BatchExecutor.generate_slurm_script()",
            f"# Config: {config_yaml_path}",
            f"# Total stimuli: {n_stimuli}",
            "",
