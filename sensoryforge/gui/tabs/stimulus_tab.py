...
class TimelineScrubberWidget(QtWidgets.QWidget):
    ...
    def set_entries(self, configs: List["StimulusConfig"]) -> None:
        ...
        needed = (
            self._HEADER_HEIGHT
            + len(self._entries) * (self._BAR_HEIGHT + self._BAR_GAP)
            + 8
        )
...
class StimulusDesignerTab(QtWidgets.QWidget):
    ...
    request_workspace = (
        QtCore.pyqtSignal()
    )  # emitted when library needed but no workspace
    ...
    def _setup_ui(self) -> None:
        ...
        # Left panel: always-visible save toolbar + scrollable params + action bar
        ...
        # Always-visible Update/Revert action bar
        action_bar = QtWidgets.QFrame()
        action_bar.setFrameShape(QtWidgets.QFrame.StyledPanel)
        ...
        self.btn_stack_update.setToolTip(
            "Commit current parameters to the selected stimulus in the set"
        )
        ...
        action_row.addWidget(self.btn_stack_update, stretch=1)
        action_row.addWidget(self.btn_stack_revert, stretch=1)
    ...
    def _build_stack_section(self) -> None:
        ...
        btn_row1.addWidget(self.btn_stack_duplicate)
    ...
    def _build_type_section(self) -> None:
        ...
        outer.setSpacing(2)
        outer.addWidget(QtWidgets.QLabel("Stimulus Type:"))
        container = QtWidgets.QWidget()
        ...
        self.control_layout.addWidget(wrapper)
    ...
    def _build_moving_subtype_section(self) -> None:
        ...
        self._lbl_circular_num_steps_row = circular_form.labelForField(
            self.spin_circular_num_steps
        )
        layout.addWidget(self.circular_params_widget)
        # Save label refs so we can hide duplicate rows for non-"moving" types
        self.lbl_circ_center_x_label = circular_form.labelForField(
            self.spin_circular_center_x
        )
        self.lbl_circ_center_y_label = circular_form.labelForField(
            self.spin_circular_center_y
        )
        self.lbl_circ_sigma_label = circular_form.labelForField(
            self.spin_circular_sigma
        )
        ...
        self._lbl_slide_num_steps_row = slide_form.labelForField(
            self.spin_slide_num_steps
        )
    ...
    def _build_spatial_section(self) -> None:
        ...
        # Motion type selector (merged from removed Motion Mode section)
        ...
        form.addRow("Motion:", self.cmb_motion_type)

        # Hidden legacy radio buttons kept for backward compatibility
        ...
        # Save label widget refs for per-type position show/hide
        self.lbl_start_x_label = form.labelForField(self.spin_start_x)
        self.lbl_start_y_label = form.labelForField(self.spin_start_y)
        self.lbl_end_x_label = form.labelForField(self.spin_end_x)
        self.lbl_end_y_label = form.labelForField(self.spin_end_y)
        self.lbl_speed_label = form.labelForField(self.spin_speed)

    def _build_temporal_section(self) -> None:
        group = CollapsibleGroupBox("Temporal Parameters", start_expanded=True)
        form = group.layout()
        form.setLabelAlignment(QtCore.Qt.AlignRight)
        ...
        self.spin_onset.setToolTip(
            "When this sub-stimulus starts in the global timeline"
        )
        ...
        self.spin_duration.setToolTip(
            "Duration of this sub-stimulus (0 = use global total)"
        )
        ...
        repeat_group = CollapsibleGroupBox(
            "Repeat Pattern", nested=True, start_expanded=False
        )
        ...
        for w in (
            self.spin_repeat_nx,
            self.spin_repeat_ny,
            self.spin_repeat_spacing_x,
            self.spin_repeat_spacing_y,
        ):
            w.setEnabled(False)

        self.btn_repeat_apply = QtWidgets.QPushButton("Apply Pattern")
        self.btn_repeat_apply.setEnabled(False)
        repeat_layout.addRow(self.btn_repeat_apply)

        group.addRow(repeat_group)
        self.control_layout.addWidget(group)
    ...
    def _build_library_section(self) -> None:
        ...
        self.btn_load_stimulus.setToolTip(
            "Load selected set into the current Stimulus Set"
        )
    ...
    def _init_expert_mode_stimulus(self) -> None:
        ...
        for w in (
            self.spin_num_steps,
            self.spin_moving_sigma,
            self.spin_circular_num_steps,
            self.spin_circular_sigma,
            self.spin_slide_num_steps,
            self.spin_slide_sigma,
        ):
            ew.append(w)
        for attr in (
            "_lbl_num_steps_row",
            "_lbl_moving_sigma_row",
            "_lbl_circular_num_steps_row",
            "lbl_circ_sigma_label",
            "_lbl_slide_num_steps_row",
            "lbl_slide_sigma_label",
        ):
            lbl = getattr(self, attr, None)
            if lbl is not None:
                ew.append(lbl)
    ...
    def _connect_signals(self) -> None:
        ...
        self.btn_stack_revert.clicked.connect(self._on_stack_revert)
        ...
        self.cmb_preview_grid.currentTextChanged.connect(self._on_preview_grid_changed)
        ...
        self.texture_subtype_combo.currentIndexChanged.connect(
            self._on_texture_subtype_changed
        )
        ...
        self.moving_subtype_combo.currentIndexChanged.connect(
            self._on_moving_subtype_changed
        )
        for spin in (
            self.spin_linear_start_x,
            self.spin_linear_start_y,
            self.spin_linear_end_x,
            self.spin_linear_end_y,
            self.spin_num_steps,
            self.spin_moving_sigma,
            self.spin_circular_center_x,
            self.spin_circular_center_y,
            self.spin_radius,
            self.spin_circular_num_steps,
            self.spin_start_angle,
            self.spin_end_angle,
            self.spin_circular_sigma,
            self.spin_slide_start_x,
            self.spin_slide_start_y,
            self.spin_slide_end_x,
            self.spin_slide_end_y,
            self.spin_slide_num_steps,
            self.spin_slide_sigma,
        ):
        ...
        for spin in (
            ...
            spin.valueChanged.connect(self._handle_preview_request)
        self.btn_repeat_apply.clicked.connect(self._on_apply_repeat_pattern)
    ...
    def _on_type_selected(self, button: QtWidgets.QAbstractButton) -> None:
        ...
        if is_texture_type:
            ...
            self.gabor_params_widget.setVisible(
                is_gabor
                or (use_combo and self.texture_subtype_combo.currentIndex() == 0)
            )
            self.edge_grating_params_widget.setVisible(
                is_grating
                or (use_combo and self.texture_subtype_combo.currentIndex() == 1)
            )
            self.noise_params_widget.setVisible(
                is_noise
                or (use_combo and self.texture_subtype_combo.currentIndex() == 2)
            )

        # Full-field types (noise, grating) don't need position/spread coords
        is_full_field = self._selected_type in ("noise", "grating")
        current_motion = (
            self.cmb_motion_type.currentText().lower()
            if hasattr(self, "cmb_motion_type")
            else "static"
        )
        for widget in (
            self.spin_start_x,
            ...
            widget.setVisible(not is_full_field)
        for lbl_attr in (
            "lbl_start_x_label",
            "lbl_start_y_label",
            "lbl_end_x_label",
        ...
        self.dbl_spread.setVisible(spread_visible)
        self.lbl_spread.setVisible(spread_visible)

        # Show/hide moving group for legacy "moving" type or trajectory motion
        is_moving_type = self._selected_type == "moving"
        needs_trajectory_ui = current_motion != "static" and not is_full_field
        self.moving_group.setVisible(is_moving_type or needs_trajectory_ui)
    ...
    def _on_repeat_toggled(self, state: int) -> None:
        ...
        self.btn_repeat_apply.setEnabled(enabled)
        self._request_preview()

    def _on_apply_repeat_pattern(self) -> None:
        """Force immediate preview update with the current repeat settings."""
        if self._loading:
            return
    ...
    def _on_stack_update_selected(self) -> None:
        """Overwrite the selected stack entry with current UI state."""
        if self._active_stack_index is None:
            return
        if not (0 <= self._active_stack_index < len(self._stimulus_stack)):
            return
    ...
    def _on_stack_revert(self) -> None:
        """Revert UI to the last committed state for the selected stimulus."""
    ...
    def _on_stack_duplicate(self) -> None:
        ...
        if (
            ...
            or index < 0
            or index >= len(self._stimulus_stack)
        ):
            return
    ...
    def _resolve_composite_grid_layer(self) -> None:
        ...
        if self._target_layer_name is None:
            ...
            densities = [
                float(l.get("density", 100.0)) for l in layers if isinstance(l, dict)
            ]
            avg_density = (
                sum(densities) / max(len(densities), 1) if densities else 100.0
            )
    ...
    def _write_stimulus(self, path: Path) -> None:
        if self._stimulus_stack:
            stimuli = [cfg.as_dict() for cfg in self._stimulus_stack]
            bundle = {
    ...
    def _load_stimulus_from_file(self, path: Path) -> None:
        ...
        try:
            ...
            if payload.get("kind") == "stimulus_stack":
                ...
                comp_mode = payload.get("composition_mode", "add")
                self._composition_mode = comp_mode
                self.cmb_composition_mode.setCurrentText(comp_mode)
                for item in payload.get("stimuli", []):
                ...
                self._refresh_stack_list()
                if self._stimulus_stack:
                    self.stimulus_stack_list.setCurrentRow(0)
                    self._apply_config(self._stimulus_stack[0])
            ...
            self.texture_subtype_combo.setCurrentIndex(
                texture_index_map.get(texture_subtype, 0)
            )

            self.spin_wavelength.setValue(float(payload.get("wavelength", 0.5)))
            self.spin_texture_orientation.setValue(
                float(payload.get("texture_orientation", 0.0))
            )
            self.spin_texture_sigma.setValue(float(payload.get("texture_sigma", 0.3)))
            self.spin_phase.setValue(float(payload.get("phase", 0.0)))
            self.spin_edge_orientation.setValue(
                float(payload.get("edge_orientation", 0.0))
            )
            ...
            self.moving_subtype_combo.setCurrentIndex(
                moving_index_map.get(moving_subtype, 0)
            )
            ...
            self.spin_circular_center_x.setValue(
                float(payload.get("circular_center_x", 0.0))
            )
            self.spin_circular_center_y.setValue(
                float(payload.get("circular_center_y", 0.0))
            )
            self.spin_radius.setValue(float(payload.get("radius", 1.0)))
            self.spin_circular_num_steps.setValue(
                int(payload.get("circular_num_steps", 100))
            )
    ...
    def _update_preview(self) -> None:
        ...
        self.timeline_scrubber.set_total_time(
            float(times[-1]) if len(times) > 0 else 300.0
        )
    ...
    def _collect_preview_configs(self) -> List[StimulusConfig]:
        ...
        if self._stimulus_stack:
            ...
            if (
                self._active_stack_index is not None
                and 0 <= self._active_stack_index < len(configs)
            ):
    ...
    def _collect_config(self) -> StimulusConfig:
        ...
        config = StimulusConfig(
            ...
            num_steps=(
                self.spin_num_steps.value()
                if self._moving_subtype == "linear"
                else (
                    self.spin_circular_num_steps.value()
                    if self._moving_subtype == "circular"
                    else self.spin_slide_num_steps.value()
                )
            ),
            ...
            moving_sigma=(
                self.spin_moving_sigma.value()
                if self._moving_subtype == "linear"
                else (
                    self.spin_circular_sigma.value()
                    if self._moving_subtype == "circular"
                    else self.spin_slide_sigma.value()
                )
            ),
    ...
    def _apply_config(self, config: StimulusConfig) -> None:
        ...
        # Update type-specific widget visibility (mirrors _on_type_selected)
        is_texture = self._selected_type == "texture"
        self.texture_group.setVisible(is_texture)
        self.gabor_params_widget.setVisible(self._selected_type == "gabor")
        self.edge_grating_params_widget.setVisible(self._selected_type == "grating")
        self.noise_params_widget.setVisible(self._selected_type == "noise")
        is_full_field = self._selected_type in ("noise", "grating")
        is_noise = self._selected_type == "noise"
        for widget in (
            self.spin_start_x,
        ...
        self.texture_subtype_combo.setCurrentIndex(
            tex_map.get(self._texture_subtype, 0)
        )
        ...
        self._target_layer_name = (
            None if actual_text in ("all", "All Grids") else actual_text
        )
        self._resolve_composite_grid_layer()
    ...
    def _build_stimulus_frames(
        ...
        for idx, t_val in enumerate(time_axis):
            ...
            if config.stimulus_type == "gaussian":
                frame = self._compute_base_frame(xx, yy, cx, cy, config)
            ...
            if config.repeat_enabled and (config.repeat_nx > 1 or config.repeat_ny > 1):
                ...
                for ix in range(nx):
                    for iy in range(ny):
                        ox = (ix - (nx - 1) / 2.0) * sx
                        oy = (iy - (ny - 1) / 2.0) * sy
                        tiled += self._compute_base_frame(
                            xx, yy, cx + ox, cy + oy, config
                        )
                frame = tiled
    ...
    def _compute_base_frame(
        ...
        """Compute a single spatial frame for position (cx, cy).

        Centralises per-type frame computation so it can be called for
        each copy when repeat tiling is enabled.

        Args:
            xx: X-coordinates [H, W]. Units: mm.
            yy: Y-coordinates [H, W]. Units: mm.
            cx: Centre x position. Units: mm.
            cy: Centre y position. Units: mm.
            config: Stimulus configuration.

        Returns:
            Frame tensor [H, W], amplitude normalised to 1.0.
        """
        st = config.stimulus_type
        if st == "gaussian":
        ...
        elif st in ("texture", "gabor", "grating", "noise"):
            override = None
            if st == "gabor":
                override = "gabor"
            elif st == "grating":
                override = "edge_grating"
            elif st == "noise":
                override = "noise"
            return self._generate_texture_frame(
                xx, yy, cx, cy, config, override_subtype=override
            )
        else:
            ...
            return gabor_texture(
                ...
                sigma=max(
                    (
                        config.spread
                        if subtype == "gabor"
                        else self.spin_texture_sigma.value()
                    ),
                    1e-6,
                ),
    ...
    def _generate_moving_frames(
        ...
        dt = max(config.dt_ms, MIN_TIME_STEP_MS)
        time_axis = torch.arange(0.0, config.num_steps * dt, dt, device=device)[
            : config.num_steps
        ]
    ...
    def _generate_motion_trajectory_frames(
        ...
        Handles circular and slide motion types paired with non-moving stimulus
        shapes (gaussian, point, edge, gabor, grating, noise).  The trajectory
        parameters are taken from the moving sub-type widgets (circular params
        use center/radius/angles; slide params use start/end).
        ...
        if mt == "circular":
            ...
            # Fallback for any other value — should not normally reach here
            trajectory = linear_motion(
                start=config.start,
                end=config.end,
                num_steps=num_steps,
        ...
        for idx in range(num_steps):
            ...
            frame = self._compute_base_frame(xx, yy, pos_x, pos_y, config)
            if config.repeat_enabled and (config.repeat_nx > 1 or config.repeat_ny > 1):
                nx, ny = config.repeat_nx, config.repeat_ny
                sx, sy = config.repeat_spacing_x, config.repeat_spacing_y
                tiled = torch.zeros_like(xx)
                for ix in range(nx):
                    for iy in range(ny):
                        ...
                        oy = (iy - (ny - 1) / 2.0) * sy
                        tiled += self._compute_base_frame(
                            xx, yy, pos_x + ox, pos_y + oy, config
                        )
                frame = tiled
