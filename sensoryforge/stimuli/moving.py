...
def velocity_profile(
    ...
    if profile_type == "constant":
        ...
        if len(profile) > num_steps:
            ...
            # Pad with ones
            profile = torch.cat(
                [profile, torch.ones(num_steps - len(profile), device=device)]
            )
...
def tap_sequence(
    ...
    if interval_duration < 1:
        raise ValueError(
            f"interval_duration must be at least 1, got {interval_duration}"
        )
    if num_taps < 1:
    ...
    for i in range(num_steps):
        ...
        if t_val <= 0:
            ...
            if idx == 0:
                ...
                modulated_trajectory[i] = (1 - alpha) * base_trajectory[
                    idx - 1
                ] + alpha * base_trajectory[idx]
