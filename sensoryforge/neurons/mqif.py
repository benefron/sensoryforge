...
class MQIFNeuronTorch(BaseNeuron):
    ...
    def get_param_spec(cls) -> List[ParamSpec]:
        ...
        return [
            ParamSpec("a", dtype="float", default=0.04, min_val=0.0, max_val=1.0,
                       step=0.01, unit="",
                       tooltip="Quadratic term gain"),
            ParamSpec("b", dtype="float", default=0.2, min_val=0.0, max_val=1.0,
                       step=0.01, unit="",
                       tooltip="Adaptation coupling to v"),
            ParamSpec("vr", dtype="float", default=-60.0, min_val=-100.0,
                       max_val=0.0, step=1.0, unit="mV",
                       tooltip="Resting/quadratic root voltage"),
            ParamSpec("vt", dtype="float", default=-40.0, min_val=-80.0,
                       max_val=0.0, step=1.0, unit="mV",
                       tooltip="Instantaneous threshold voltage"),
            ParamSpec("v_reset", dtype="float", default=-60.0, min_val=-100.0,
                       max_val=0.0, step=1.0, unit="mV",
                       tooltip="Post-spike reset voltage"),
            ParamSpec("v_peak", dtype="float", default=30.0, min_val=-20.0,
                       max_val=60.0, step=1.0, unit="mV",
                       tooltip="Spike detection voltage"),
            ParamSpec("d", dtype="float", default=2.0, min_val=0.0, max_val=20.0,
                       step=0.5, unit="",
                       tooltip="Post-spike adaptation increment"),
            ParamSpec("tau_m", dtype="float", default=10.0, min_val=0.5,
                       max_val=200.0, step=0.5, unit="ms",
                       tooltip="Membrane time constant"),
            ParamSpec("tau_u", dtype="float", default=100.0, min_val=1.0,
                       max_val=500.0, step=5.0, unit="ms",
                       tooltip="Adaptation time constant"),
            ParamSpec("v_init", dtype="float", default=-60.0, min_val=-100.0,
                       max_val=50.0, step=1.0, unit="mV",
                       tooltip="Initial membrane voltage (default: vr)"),
            ParamSpec("u_init", dtype="float", default=0.0, min_val=-20.0,
