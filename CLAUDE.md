...
- **Seeded innervation crashes on MPS and CUDA** — innervation draws with a per-instance CPU `torch.Generator` (F-006 fix) but on device tensors, raising `Expected a 'mps' device type for generator but found 'cpu'`; `SimulationEngine(device="mps")` with a seeded population fails. Use `device="cpu"` until task E6 lands. (F-038)
- **CLI and batch runs of canonical configs ignore `simulation.dt_ms` when generating stimuli** — the adapter still reads the old `dt` key, and trapezoid/gaussian/step/ramp stimuli use the legacy `temporal.dt` of 0.1 ms (the trapezoid also ignores `--duration`); `sensoryforge run` on a `dt_ms: 1.0` config with `--duration 100` produces 10,450 bins that the engine treats as 1 ms each. (F-039, F-040, F-024)
- **GUI exports always write `simulation.dt_ms: 1.0`**, whatever step the GUI simulated at (0.1 ms by default), so exported configs run with different filter integration and neuron sub-steps in the CLI. (F-041)
- **`dt_ms` must be a whole multiple of `integrate_dt_ms` (0.05 ms)** — otherwise neuron time is silently rescaled (0.12 ms bins integrate 0.10 ms). Not yet validated. (F-042)
