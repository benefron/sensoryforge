...
- **The TouchSim comparison agrees only in part** (`benchmarks/results/touchsim_comparison/`, one fitted parameter): RA's silent hold and release response, and the Izhikevich SA rate-intensity curve, agree; RA is far less sensitive relative to SA than TouchSim's RA, SA's ramp response is 2-3x weaker than SA1's, and AdEx SA has a hard threshold. `tests/validation/test_touchsim_comparison.py` fails when any verdict changes.
...
- **Voltage clamp divergence from pressure-simulation** under strongly negative drive (F-037, settled: the tactile recipes never reach the floor, and `tests/integration/test_recipe_calibration.py` fails if one comes within 20 mV of it).
