...
- **The tactile recipes are fitted to TouchSim's SA1/RA afferents** (D-f4d0967, D-d9bd411; `scripts/validation/fit_afferents.py`): SA filter `k2` default 8.0 (was 3.0), Izhikevich `d` SA 15 / RA 24 in the recipe, refitted AdEx `SA1_tonic`/`RA1_phasic` presets. Both recipes agree with TouchSim on SA's ramp rate and RA's onset rate and silent hold, and Izhikevich also on SA's hold rate at every depth (AdEx SA holds at 2.9 Hz against 8.6 at 0.2 mm, one spike against three) (`benchmarks/results/touchsim_comparison/`, one fitted parameter; `tests/validation/test_touchsim_comparison.py` fails when any verdict changes). They disagree on release: RA's release is as strong as its onset (the RA filter is symmetric in the rate of change). A tactile afferent's neuron input is floored at 0 mA (`PopulationConfig.input_floor`, resolved by `config/defaults.py::resolve_input_floor`; the recorded `filtered` stays signed; D-43dc520), and AdEx's SA1/RA1 presets have a 2 ms refractory period (`t_ref`, D-f5853a4). **After strong stimulation AdEx's adaptation drives the voltage far below rest** (about -380 mV without the -130 mV clamp); see the open ledger entry before trusting AdEx dynamics there.
- **Every stimulus defaults to peak amplitude 1.0 and renders non-negative** (D-0437899; `signed: true` restores the signed Gabor/texture). With the calibrated recipes every type fires at its defaults, except `gabor`, whose default geometry (sigma 0.3 mm, wavelength 0.5 mm) covers about two receptors of a 0.15 mm grid (F-083 closed; the gabor geometry is open).
- **The test suite runs with the cyclic GC on and collects after every `gui` test** (conftest autouse fixture). A Python-owned window kept alive by a reference cycle is deleted by whichever collection frees the cycle, even mid-slot (F-085, closed): connect Qt signals in GUI code to bound methods, which PyQt holds weakly, never to `functools.partial(self.method, ...)` or a lambda over `self`, which make that cycle (pyqtgraph signals still go through `plot_factory.connect` with plain functions). `tests/gui_v2/test_window_lifetime.py` checks that a dropped window is freed without the collector.
- **Voltage clamp divergence from pressure-simulation** under strongly negative drive (F-037, settled): the Izhikevich recipe's SA does reach the -120 mV floor on trailing edges, but its spikes are identical with and without the clamp, and `tests/integration/test_recipe_calibration.py` fails if they ever differ.
