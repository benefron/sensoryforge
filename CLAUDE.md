...
- **The tactile recipes are fitted to TouchSim's SA1/RA afferents** (D-f4d0967, D-d9bd411; `scripts/validation/fit_afferents.py`): SA filter `k2` default 8.0 (was 3.0), Izhikevich `d` SA 15 / RA 24 in the recipe, refitted AdEx `SA1_tonic`/`RA1_phasic` presets. Both recipes agree with TouchSim on SA's hold and ramp rates and RA's onset rates and silent hold (`benchmarks/results/touchsim_comparison/`, one fitted parameter; `tests/validation/test_touchsim_comparison.py` fails when any verdict changes). They disagree on release: RA's release is as strong as its onset (the RA filter is symmetric in the rate of change). **At its calibrated gains the AdEx recipe's voltage leaves the physiological range** (about -1000 mV without the -130 mV clamp); see the open ledger entry before trusting AdEx dynamics.
