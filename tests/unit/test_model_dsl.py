...
class TestNeuronModelParsing:
    ...
    def test_threshold_variable_not_state_var_error(self):
        ...
        with pytest.raises(
            ValueError, match="Threshold variable.*is not a state variable"
        ):
            NeuronModel(
                equations="dv/dt = -v + I",
    ...
    def test_reset_variable_not_state_var_error(self):
        ...
        with pytest.raises(
            ValueError, match="Reset rule variable.*is not a state variable"
        ):
            NeuronModel(
                equations="dv/dt = -v + I",
                threshold="v >= 1.0",
...
class TestNeuronModelCompilation:
    ...
    def test_unsupported_solver_error(self):
        ...
        model = NeuronModel(
            equations="dv/dt = -v + I", threshold="v >= 1.0", reset="v = 0.0"
        )
...
class TestEdgeCases:
    ...
    def test_empty_equations_error(self):
        ...
        with pytest.raises(ValueError, match="No valid differential equations found"):
            NeuronModel(equations="", threshold="v >= 1.0", reset="v = 0.0")
