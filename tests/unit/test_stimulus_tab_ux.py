...
def test_stack_save_preserves_stack(stim_tab, tmp_path):
    ...
    assert (
        len(stim_tab._stimulus_stack) >= 2
...
def test_repeat_spinbox_triggers_preview_timer(stim_tab):
    ...
    assert (
        stim_tab._preview_timer.isActive()
...
def test_stimulus_tab_has_expert_mode_checkbox(stim_tab):
    ...
    assert hasattr(
        stim_tab, "chk_expert_mode"
    ), "StimulusDesignerTab missing chk_expert_mode"


def test_expert_only_widgets_non_empty(stim_tab):
    """_expert_only_widgets_stimulus must contain at least one widget."""
    assert (
        len(stim_tab._expert_only_widgets_stimulus) > 0
...
def test_basic_mode_hides_expert_widgets(stim_tab):
    ...
    for w in stim_tab._expert_only_widgets_stimulus:
        assert (
            w.isHidden()
        ), f"Widget {w!r} should be hidden in Basic mode but is visible"
...
def test_expert_mode_shows_advanced_widgets(stim_tab):
    ...
    for w in stim_tab._expert_only_widgets_stimulus:
        assert (
            not w.isHidden()
        ), f"Widget {w!r} should be visible in Expert mode but is hidden"
...
def test_revert_restores_committed(stim_tab):
    ...
    stim_tab._on_stack_update_selected()
    committed_amp = (
        stim_tab._committed_config.amplitude if stim_tab._committed_config else 2.5
    )
    ...
    assert (
        abs(reverted_amp - committed_amp) < 0.01
...
def test_dirty_indicator_appears_on_edit(stim_tab):
    ...
    assert (
        "*" in stim_tab.btn_stack_update.text()
...
def test_dirty_indicator_clears_on_save(stim_tab):
    ...
    stim_tab._on_stack_update_selected()
    assert (
        "*" not in stim_tab.btn_stack_update.text()
...
def test_gaussian_type_shows_spread_hides_texture_group(stim_tab):
    ...
    assert (
        stim_tab.texture_group.isHidden()
    ), "texture_group should be hidden for gaussian type"
    assert (
        not stim_tab.dbl_spread.isHidden()
    ), "spread should not be hidden for gaussian type"
...
def test_gabor_type_shows_texture_params(stim_tab):
    ...
    assert (
        not stim_tab.texture_group.isHidden()
    ), "texture_group must not be hidden for gabor type"
    assert (
        not stim_tab.gabor_params_widget.isHidden()
    ), "gabor_params_widget must not be hidden for gabor type"
    assert (
        stim_tab.edge_grating_params_widget.isHidden()
    ), "edge_grating_params_widget must be hidden for gabor type"
    assert (
        stim_tab.noise_params_widget.isHidden()
    ), "noise_params_widget must be hidden for gabor type"
    # Subtype combo must be hidden for direct gabor type
    assert (
        stim_tab.texture_subtype_combo.isHidden()
...
def test_grating_type_shows_grating_params(stim_tab):
    ...
    assert (
        not stim_tab.texture_group.isHidden()
    ), "texture_group must not be hidden for grating type"
    assert (
        not stim_tab.edge_grating_params_widget.isHidden()
    ), "edge_grating_params_widget must not be hidden for grating type"
    assert (
        stim_tab.gabor_params_widget.isHidden()
...
def test_noise_type_shows_noise_params(stim_tab):
    ...
    assert (
        not stim_tab.texture_group.isHidden()
    ), "texture_group must not be hidden for noise type"
    assert (
        not stim_tab.noise_params_widget.isHidden()
