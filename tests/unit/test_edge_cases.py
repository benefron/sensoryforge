...
def test_zero_amplitude_no_nan():
    ...
    for key in (
        ...
        assert not torch.isnan(
            tensor
        ).any(), f"NaN in {key} with zero-amplitude stimulus"
        assert not torch.isinf(
            tensor
        ).any(), f"Inf in {key} with zero-amplitude stimulus"
...
def test_extreme_amplitude_no_nan(factor):
    ...
    for key in (
        ...
        assert not torch.isnan(
            tensor
        ).any(), f"NaN in {key} at amplitude={30.0 * factor}"
        assert not torch.isinf(
            tensor
        ).any(), f"Inf in {key} at amplitude={30.0 * factor}"
...
def test_large_sigma_innervation_no_nan():
    ...
    for key in ("sa_spikes", "ra_spikes"):
        ...
        assert not torch.isnan(
            tensor
        ).any(), f"NaN in {key} with wide-sigma innervation"
